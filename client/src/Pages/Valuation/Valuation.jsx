...
import { submitValuationCoa, valuationCoaChangeStatus, pickValuationBraintreePayload } from '../../services/forumService';
...
const Valuation = () => {
    ...
    const { brands: authBrands = [] } = useAppSelector((state) => state.authenticationRequest || {});
    ...
    const onSubmit = async (data) => {
        ...
        try {
            if (isTestMode) {
                ...
                setSubmitSuccess(res?.msg ?? 'Change-status API called successfully (test).');
            ...
            if (order_number && transaction_id && token) {
                ...
                setSubmitSuccess(res?.msg ?? 'Valuation request submitted successfully.');
                setSubmitStatus('succeeded');
                reset();
                return;
            }
            ...
            const message =
                (data && (data.msg ?? data.message ?? data.error ?? data.additionalMsg)) ||
                err?.message ||
                'Submission failed. Please try again.';
            setSubmitError(typeof message === 'string' ? message : 'Submission failed. Please try again.');
            setSubmitStatus('failed');
        }
    };
                ...
                <div className="bg-white rounded-[22px] shadow-lg border border-gray-200 overflow-hidden">
                    <div className="p-6 sm:p-8 md:p-10 lg:p-12">
                        ...
                        <div className="mb-6 sm:mb-8 flex items-center justify-between flex-wrap gap-2">
                            <h1 className="text-2xl sm:text-3xl font-bold text-primary">What's it worth?</h1>
                        </div>

                        {/* Description */}
                        <div className="mb-6 sm:mb-8 space-y-4">
                            <p className="text-sm sm:text-base text-gray-600 leading-relaxed">
                                Our valuation service provides an an expert assessment of your item's market value. Ideal for buyers and sellers, it ensures fair transactions and informed pricing. Our experts analyse market trends, comparable sales, and condition to deliver an accurate valuation.
                            </p>
                            <p className="text-sm sm:text-base text-gray-600 leading-relaxed">
                                In order to purchase this add on, a previous certificate of authenticity from Authentic Detective is required. <span className="font-bold">Price - $10</span>
                            </p>
                        </div>
                        ...
                        {showPopup && (
                            ...
                            >
                                ...
                                <div className="relative w-full max-w-md rounded-2xl bg-white shadow-xl border border-gray-200 p-6 sm:p-8">
                                    <h2 id="popup-title" className={`text-lg font-semibold mb-4 text-center ${isError ? 'text-red-600' : 'text-gray-900'}`}>
                                ...
                                <div>
                                    <Controller
                                        ...
                                        render={({ field }) => {
                                            ...
                                            const brandsSource = (Array.isArray(apiBrands) && apiBrands.length
                                                ? apiBrands
                                                : Array.isArray(authBrands)
                                                    ? authBrands
                                                    : []);
                                            const sortedBrands = [...brandsSource].sort((a, b) => {
                                                const nameA = (a.brand || a.name || a.brand_name || '').toString().toLowerCase();
                                                const nameB = (b.brand || b.name || b.brand_name || '').toString().toLowerCase();
