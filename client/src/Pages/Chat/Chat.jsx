...
import { FiChevronLeft, FiPaperclip, FiSearch, FiSend, FiX } from 'react-icons/fi';
...
const DUMMY_CHATS = [
  {
    ...
    unread: 1,
    messages: [
      { id: 'm1', from: 'them', text: 'Hi! How can we help you with your authentication today?', time: '2:38 AM' },
      { id: 'm2', from: 'me', text: 'I have a question about my certificate.', time: '2:38 AM' },
      { id: 'm3', from: 'them', text: 'Sure—share your order number and we’ll look it up.', time: '2:39 AM' },
    ],
  },
  {
    id: 'sarah',
    ...
    unread: 0,
    messages: [
      { id: 'm1', from: 'them', text: 'Thanks for the quick verification on my order.', time: '1:02 AM' },
      { id: 'm2', from: 'me', text: 'Glad we could help. Anything else you need?', time: '1:04 AM' },
      { id: 'm3', from: 'them', text: 'Not right now, thanks!', time: '1:06 AM' },
    ],
  },
  {
    id: 'james',
    ...
    unread: 0,
    messages: [
      { id: 'm1', from: 'them', text: 'When will my certificate be ready?', time: 'Yesterday' },
      { id: 'm2', from: 'me', text: 'It’s under review. We’ll email you within 24 hours.', time: 'Yesterday' },
    ],
  },
];
...
const Chat = () => {
  const [selectedChatId, setSelectedChatId] = useState(DUMMY_CHATS[0]?.id ?? null);
  ...
  const chats = useMemo(() => {
    ...
    return DUMMY_CHATS.filter((c) => {
      const hay = `${c.title} ${c.subtitle ?? ''} ${c.lastMessage ?? ''}`.toLowerCase();
                ...
                <div className="bg-primary text-secondary px-4 py-3 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-semibold leading-tight truncate">Chat</div>
                      <div className="text-xs text-secondary/80 truncate">Your conversations</div>
                    </div>
                  </div>
                </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <div className="text-[11px] text-primary/60 flex-shrink-0">{c.time}</div>
                            </div>
                            <div className="flex items-center justify-between gap-2 mt-0.5">
                              <div className="text-xs text-primary/70 truncate">{c.lastMessage}</div>
                              {c.unread ? (
                        ...
                        <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          <div
                            className={`max-w-[85%] sm:max-w-[75%] px-3 py-2 border border-primary/10 shadow-sm rounded-none ${
                              isMe ? 'bg-primary text-secondary' : 'bg-secondary text-primary'
                            }`}
                          >
                            {m.attachmentName ? (
                              <div className={`text-xs mb-1 ${isMe ? 'text-secondary/80' : 'text-primary/60'}`}>
                                Attachment: {m.attachmentName}
                              </div>
                            ) : null}
                            <div className="text-sm leading-relaxed break-words">{m.text}</div>
                            <div className={`mt-1 text-[11px] ${isMe ? 'text-secondary/70' : 'text-primary/50'}`}>
                              {m.time}
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="max-w-3xl mx-auto">
                    {attachedFile && (
                      <div className="mb-2 flex items-center justify-between gap-3 bg-[#F5F5F0] border border-primary/10 px-3 py-2 rounded-none">
                        <div className="min-w-0">
                          <div className="text-xs text-primary/60">Selected file</div>
                          <div className="text-sm font-medium text-primary truncate">{attachedFile.name}</div>
                        </div>
                        <button
                          type="button"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 min-w-0 bg-[#F5F5F0] border border-primary/10 rounded-2xl h-11 px-3 flex items-center">
                        <textarea
                          ...
                          className="w-full h-7 resize-none overflow-hidden bg-transparent text-sm sm:text-[15px] text-primary placeholder:text-primary/50 focus:outline-none leading-6"
                          aria-label="Message"
                        />
                      </div>
