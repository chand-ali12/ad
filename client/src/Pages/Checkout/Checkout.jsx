...
import { verifyCoupon, clearCoupon, getAuthCheckoutBraintreeToken, setError, getAuthenticityCardPricing, submitBraintreeAuthCards, submitBraintreeCheckout, submitBraintreeValuation } from '../../store/slices/checkoutSlice';
import { clearCart } from '../../store/slices/cartSlice';
import { valuationCoaChangeStatus } from '../../services/forumService';

const BRAINTREE_SCRIPT = 'https://js.braintreegateway.com/web/dropin/1.36.0/js/dropin.min.js';
...
const Checkout = () => {
    ...
    const { coupon, couponError, couponStatus, cardPricing, cardPricingStatus, status: checkoutStatus, error: checkoutError } = useAppSelector((state) => state.checkout);
    const { items: cartItems = [], subtotal: cartSubtotal = 0 } = useAppSelector((state) => state.cart) ?? {};

    const { register, handleSubmit, formState: { errors }, reset, getValues, setValue, setError, clearErrors } = useForm({
    ...
    const rawIds = location.state?.certificateIds ?? cartItems.map((i) => i.id ?? i.coa_number).filter(Boolean);
    const certificateIds = rawIds.map((id) => (id != null && typeof id === 'object' ? (id.id ?? id.query_id ?? id.certificate_id ?? id.coa_number) : id)).filter((id) => id != null && id !== '');
    ...
    useEffect(() => {
        ...
        if ((checkoutType === 'auth' || pageName === 'bulk' || pageName === 'auth-cards' || payload?.query_type === 'auth') && payload) {
            ...
            if (token) {
                setBraintreePayload({
                    ...payload,
                    token,
                    query_type: payload?.query_type ?? (pageName === 'auth-cards' ? 'auth-cards' : 'auth'),
                    encrypt_amount: encryptedPayload ?? payload?.encryptedAmount,
                    encryptedAmount: encryptedPayload ?? payload?.encryptedAmount,
                });
            }
        }
    }, [checkoutType, location.state?.braintreePayload, location.state?.page, searchParams]);
    ...
    useEffect(() => {
        ...
        if (fn != null && String(fn).trim()) setValue('firstName', String(fn).trim());
        if (ln != null && String(ln).trim()) setValue('lastName', String(ln).trim());
    ...
    const pricingTier = Array.isArray(cardPricing) && coaCount > 0
        ? cardPricing.find((t) => Number(t.qty) === coaCount && Number(t.type) === 1)
        || cardPricing.find((t) => Number(t.qty) === coaCount)
        : null;
    const subtotalFromPricing = pricingTier != null ? Number(pricingTier.amount) || 0 : 0;
    const subtotalFromCart = cartItems.length
        ? cartItems.reduce((sum, i) => sum + (Number(i.price) || 0) * (i.quantity || 1), 0)
        : 0;
    let subtotal = cartItems.length > 0 ? subtotalFromCart : (subtotalFromPricing || subtotalFromCart);
    ...
    if (!cartItems.length && braintreePayload?.amount != null && pageNameForTotals === 'auth-cards') {
    ...
    const onSubmit = async (data) => {
        ...
        if (!certificateIds?.length) {
            dispatch(setError('No certificates to checkout. Please add items from the Authentication page.'));
            return;
        }
        try {
            const result = await dispatch(getAuthCheckoutBraintreeToken({
            ...
            const token = payload?.token ?? payload?.client_token;
            const encryptedPayload = payload?.encrypt_amount ?? payload?.encrypted_amount ?? payload?.encryptedAmount ?? payload?.encrypted_order;
            ...
            const isDecryptError = /payload is invalid|DecryptException/i.test(String(msg));
    ...
    useEffect(() => {
        ...
        loadScript(BRAINTREE_SCRIPT)
            .then(() => {
                if (!window.braintree || !document.getElementById('braintree-dropin-container')) return;
                window.braintree.dropin.create(
                    ...
                    (err, inst) => {
                        if (err) {
                            ...
                            dispatch(setError(err?.message || 'Payment form could not load.'));
                            return;
                        }
                        instance = inst;
    ...
    const onBraintreeSubmit = async (e) => {
        ...
        if (checkoutType !== 'valuation' && (encryptValue == null || encryptValue === '')) {
            dispatch(setError('Payment session expired. Please fill the form and click Complete Order again.'));
            return;
        }
        try {
            ...
            const body = {
                ...
                query_type: braintreePayload.query_type ?? (checkoutType === 'valuation' ? 'valuation' : undefined),
                ...
                ...(braintreePayload.coupon_code && { coupon_code: braintreePayload.coupon_code }),
                ...(braintreePayload.email != null && braintreePayload.email !== '' && { email: braintreePayload.email }),
                ...(braintreePayload.brand_name != null && braintreePayload.brand_name !== '' && { brand_name: braintreePayload.brand_name }),
                ...(checkoutType === 'valuation' && braintreePayload.user && { user: braintreePayload.user }),
                ...(checkoutType === 'valuation' && braintreePayload.name && { name: braintreePayload.name }),
                ...(checkoutType === 'valuation' && braintreePayload.payed_amount != null && { payed_amount: braintreePayload.payed_amount }),
                ...(checkoutType !== 'valuation' && braintreePayload.payed_amount != null && { payed_amount: braintreePayload.payed_amount }),
            };
            if (checkoutType === 'valuation') {
                ...
                const successMsg = result?.msg || 'Your order has been submitted successfully!';
    ...
    const useAuthCheckout =
        ...
        (braintreePayload?.query_type === 'auth' && pageNameForMode !== 'auth-cards');
    // ad-old: Checkout never calls "prepare" API; payload must come from Authentication/Cart. If missing, show message.
    const authCheckoutWithoutPayload = !braintreePayload && checkoutType !== 'valuation';
    ...
    return (
        <div className="w-full min-h-screen bg-[#F5F5F0] py-8 sm:py-12 md:py-16">
            <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="bg-white rounded-[22px] shadow-lg border border-gray-200 overflow-hidden" style={{ borderRadius: '22px' }}>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                            ...
                            <div className="lg:col-span-2">
                                {authCheckoutWithoutPayload && !paymentSuccessMessage && (
                                    <div className="p-6 rounded-lg bg-amber-50 border border-amber-200">
                                        <p className="text-primary font-medium mb-2">Payment could not be prepared.</p>
                                        <p className="text-sm text-gray-700 mb-4">Please start from the Authentication page, submit your item(s), and use &quot;Proceed to checkout&quot; from there.</p>
                                        <button
                                            type="button"
                                ...
                                {isValuationCheckout && (
                                    <div>
                                        <h2 className="text-xl sm:text-2xl font-bold text-primary mb-4">Valuation – Complete payment</h2>
                                        <div className="mb-6">
                                            <h3 className="text-lg font-semibold text-primary mb-3 sm:mb-4">Customer information</h3>
                                ...
                                {!isValuationCheckout && !authCheckoutWithoutPayload && (
                                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                                        ...
                                        <div>
                                            <h2 className="text-xl sm:text-2xl font-bold text-primary mb-4 sm:mb-6">Customer information</h2>
                                        ...
                                        {!showBraintreeStep && (
                                        <div>
                                            <h2 className="text-xl sm:text-2xl font-bold text-primary mb-4 sm:mb-6">Payment method</h2>

                                            <div className="mb-6 border rounded-lg overflow-hidden" style={{ borderWidth: '1px', borderColor: '#ADADAD' }}>
                                                <label className="flex items-center gap-3 cursor-pointer p-4 bg-white">
                                                    ...
                                                    />
                                                    <span className="text-base sm:text-lg text-primary font-medium">Pay with Debit/ Credit Card</span>
                                                </label>
                                                ...
                                                <div className="bg-[#EDEAE5] rounded-lg p-4 space-y-4">
                                                    ...
                                                    <div className="relative">
                                                        <input
                                                            ...
                                                            className={`w-full px-4 py-3 pr-12 rounded-lg border ${errors.cardNumber ? 'border-red-500' : 'border-gray-300'
                                                    ...
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                        <div>
                                                            <input
                                                                ...
                                                                className={`w-full px-4 py-3 rounded-lg border ${errors.expirationDate ? 'border-red-500' : 'border-gray-300'
                                                                    } focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-primary placeholder:text-gray-400 bg-white`}
                                                            />
                            ...
                            <div className="lg:col-span-1">
                                ...
                                <div className="bg-[#EDEAE5] rounded-lg p-6 sm:p-8 h-fit sticky top-4">
                                    {/* Order Summary */}
                                    <div className={`mb-6 flex flex-col ${isValuationCheckout ? '' : 'min-h-[400px]'}`}>
                                        {!isValuationCheckout && (
                                            <>
                                                <div className="flex justify-between items-center mb-2">
                                                    <span className="text-sm sm:text-base font-semibold text-[#767676]">SUBTOTAL</span>
                                                </div>
                                                {cartItems.length > 0 ? (
                                                    cartItems.map((item, idx) => {
                                                        const lineTotal = (Number(item.price) || 0) * (item.quantity || 1);
                                                        const label = item.brand ?? item.model ?? `Item ${idx + 1}`;
                                                        return (
                                                            <div key={item.id ?? idx} className="mb-2">
                                                                <div className="flex justify-between items-center">
                                                                    <span className="text-sm sm:text-base text-black">{label}</span>
                                                                    <span className="text-sm sm:text-base font-semibold text-black">${lineTotal.toFixed(2)}</span>
                                                                </div>
                                                            </div>
                                                        );
                                                    })
                                                    ...
                                                    <div className="mb-2">
                                                        <div className="flex justify-between items-center">
                                                            <span className="text-sm sm:text-base text-black">Order</span>
                                                            <span className="text-sm sm:text-base font-semibold text-black">${subtotal.toFixed(2)}</span>
                                                        </div>
                                                    </div>
                                                )}
                                                {discount > 0 && (
                                        ...
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            <span className="text-base sm:text-lg font-bold text-black">${(isValuationCheckout ? (braintreePayload?.amount ?? 0) : total).toFixed(2)}</span>
                                        </div>
                                        ...
                                        <div className="mb-0 -mx-4 sm:-mx-6 md:-mx-8 px-4">
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className={`w-full px-2.5 py-2.5 pr-16 rounded-lg border focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-primary placeholder:text-gray-400 bg-white ${couponError ? 'border-red-500' : 'border-gray-300'
                                                        }`}
                                                />
                                                <button
                                                    type="button"
                                    ...
                                    {!showBraintreeStep && !authCheckoutWithoutPayload && (
                                        ...
                                        >
                                            {checkoutStatus === 'loading' ? 'Processing...' : 'Complete Order'}
                                        </button>
                                    )}
                                    ...
                                    {showBraintreeStep && (
                                        <div className="mt-4">
                                            <button
                                                ...
                                                disabled={!braintreeReady || checkoutStatus === 'loading'}
                                                className="w-full bg-primary text-secondary py-2 sm:py-2 rounded-lg font-semibold text-base sm:text-lg hover:bg-primary-hover transition-colors shadow-md disabled:opacity-60 disabled:cursor-not-allowed"
                                            >
                                                {checkoutStatus === 'loading' ? 'Processing...' : 'Confirm & pay'}
                                            </button>
                                        </div>
                                    )}
                                </div>
