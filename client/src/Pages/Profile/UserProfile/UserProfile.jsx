...
import { getUserProfile, getUserCertificate, getUserQueries, updateCertificateNote } from '../../../store/slices/profileSlice';

import { getBusinessProfile } from '../../../store/slices/businessSlice';

import { submitReviewReply, deleteReview, deleteReviewReply, updateReviewReply } from '../../../store/slices/reviewsSlice';
import { uploadImage } from '../../../store/slices/uploadSlice';
import { changePaymentStatus, isSold } from '../../../store/slices/checkoutSlice';

import ProfileSection from '../../../sections/Profiles/UserProfile/ProfileSection/ProfileSection';

import CertificatesofAuthenticity, { getCertificatePdfUrl } from '../../../sections/Profiles/UserProfile/CertificatesofAuthenticity/CertificatesofAuthenticity';
import { getCoaPdf, getCertificatePdfByUuid } from '../../../services/forumService';
import { updateRequestMoreImagesCertificate, userUpdateQueryImages } from '../../../services/requestMoreImagesService';
...
const UserProfile = () => {
    ...
    const { user, status, error, businessReviewCount, certificates, queries, queryCounts } = useAppSelector(
    ...
    const handleConfirmEditReply = async () => {
        ...
        try {
            const result = await dispatch(updateReviewReply({
    ...
    const handlePaymentStatusChange = async (queryId) => {
        try {
            const result = await dispatch(changePaymentStatus({ id: queryId })).unwrap();
    ...
    const handleUpdateNote = async (certificateId, note) => {
        ...
        try {
            await dispatch(updateCertificateNote({ query_id: certificateId, note })).unwrap();
    ...
    const handleViewCertificatePdf = async (certificateUuid) => {
        ...
        try {
            const blob = await getCertificatePdfByUuid({ uuid: certificateUuid, token: authToken });
            const url = URL.createObjectURL(blob);
            window.open(url, '_blank', 'noopener,noreferrer');
            setTimeout(() => URL.revokeObjectURL(url), 60000);
        } catch (err) {
    ...
    const handleShareCoaPdf = async (certificate) => {
        const title = `Certificate ${certificate?.order ?? certificate?.coa_number ?? ''}`.trim();
        const directUrl = getCertificatePdfUrl(certificate);
        try {
            if (typeof navigator !== 'undefined' && navigator.share) {
                try {
                    ...
                    await navigator.share({ title: title || 'COA', text: title || 'Certificate of Authenticity', files: [file] });
                    return;
                } catch (e) {
                    if (e && e.name === 'AbortError') return;
                }
                if (directUrl) {
                    await navigator.share({ title: title || 'COA', text: title || 'Certificate of Authenticity', url: directUrl });
                    return;
                }
            }
    ...
    const handleMarkAsSold = async (certificate) => {
        ...
        try {
            ...
            setToastMessage(result?.msg || 'Marked as sold.');
            setToastVariant('success');
            setShowToast(true);
    ...
    const handleUploadImagesForModal = async (files) => {
        ...
        for (const file of files) {
            const result = await dispatch(uploadImage({ image: file, storage_type: 'authenticateImage' })).unwrap();
            const path = result?.data != null
                ? (typeof result.data === 'string' ? result.data : (result.data?.path ?? result.data?.url ?? result.data?.file_name ?? ''))
                : '';
    ...
    const handleRequestMoreImages = async (certificate, imagesStr) => {
        if (!certificate?.id || !imagesStr) throw new Error('Certificate and images are required');
        const isAdminRequested = certificate.request_more_images?.some((item) => Number(item?.status) === 0) ?? false;
        try {
            if (isAdminRequested) {
                await updateRequestMoreImagesCertificate({ authenticate_id: certificate.id, images: imagesStr, token: authToken });
            } else {
                await userUpdateQueryImages({ authenticate_id: certificate.id, images: imagesStr, token: authToken });
            }
    ...
    const handleConfirmDeleteReply = async () => {
        ...
        try {
            const result = await dispatch(deleteReviewReply({
    ...
    const handleReplySubmit = async (reviewId, replyText) => {
        ...
        try {
            const result = await dispatch(submitReviewReply({
                id: reviewId,
