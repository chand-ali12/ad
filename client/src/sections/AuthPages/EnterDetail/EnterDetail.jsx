...
const EnterDetail = ({ className = "" }) => {
  ...
  const coaCountNum = coaCountValue
    ? Math.max(0, parseInt(coaCountValue, 10) || 0)
    : 0;
  ...
  useEffect(() => {
    ...
    const maxQty = activeTiers.length
      ? Math.max(...activeTiers.map((t) => t.qty))
      : 0;
    const lastTierPrice = maxQty
      ? activeTiers.find((t) => t.qty === maxQty)?.amount || 0
      : 0;
  ...
  useEffect(() => {
    const quantityFromNav =
      location.state?.quantity ?? location.state?.coaCount;
  ...
  const onSubmit = async (data) => {
    const count = data.coaCount
      ? Math.max(0, parseInt(data.coaCount, 10) || 0)
      : 0;
    const coaNumbers = [];
    ...
    //   console.log("Error is :- ", err);
      setToastMessage("Certificate number not exists, please enter correct certificate number.");
