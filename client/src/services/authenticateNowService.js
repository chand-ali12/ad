...
export const submitAuthenticateNow = async ({
  ...
  formData.append(
    "image",
    Array.isArray(image) ? image.join(",") : String(image),
  );
...
export const getQueryPrice = async ({
  ...
  return request("/ad/get-query-price-new", {
    method: "POST",
    body,
    headers: {
      "Content-Type": "application/json",
...
export const bundleQueryFormSubmit = async ({
  ...
  // Build URL with query params
  let url = "/ad/bundle-query-form-submit";
  if (is_expedited !== undefined) {
    url += `?is_expedited=${is_expedited}`;
  }
