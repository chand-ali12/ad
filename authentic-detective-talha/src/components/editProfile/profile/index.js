...
import { editBusinessProfileSchema } from "../../../../utils/validationSchemas/contactsUsValidationSchema";
...
const EditProfile = () => {
  ...
  const {
    ...
    resolver: yupResolver(editBusinessProfileSchema),
    mode: "onChange",
  });
  ...
  const getBusinessProfile = async () => {
    try {
      ...
      if (response?.data) {
        ...
        setProfileImage(businessData.business_profile_picture || "");
        setCoverImage(businessData.business_cover_picture || "");

        if (userData.profile_picture) {
          setLocalProfileUrl(
            `${process.env.NEXT_PUBLIC_MEDIA_BASE_URL}/businessProfile/${userData.business_profile_picture}`
          );
        }
        if (userData.cover_picture) {
          setLocalCoverUrl(
            `${process.env.NEXT_PUBLIC_MEDIA_BASE_URL}/businessCover/${userData.business_cover_picture}`
          );
        }

        setBusinessProfiles({
          profilePicture: businessData.business_profile_picture || "",
          coverPicture: businessData.business_cover_picture || "",
        });
