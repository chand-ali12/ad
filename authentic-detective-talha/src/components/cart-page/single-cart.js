...
const SingleCartComponent = ({ realTimeUserInfo }) => {
  ...
  const submitBulkQuery = async () => {
    ...
    // Apply the same logic as in the form
    if (bulkFormsData.length === 1) {
      // Single item - use the same logic as form
      const data = bulkFormsData[0];
      ...
      const singleFormData = {
        ...
        is_user_paid: totalAmount === 0 ? 0 : 1,
      ...
      debugger;
      try {
        const response = await axiosInstance.post(endpoint, singleFormData);
        if (response.data.status_code === 200) {
          ...
          if (totalAmount === 0) {
            ...
            router.push({
              pathname: "/checkout",
              query: { data: encodedData, page: "single" },
            });
          }
    ...
    // For multiple items or paid single item, use existing bulk logic
    // const endPoint =
    ...
    //       ? item?.selectCategory?.price + COMMON_VALUE_FOR_VALUATION
    ...
    //         : 0
    //       : item?.marketValuationCheckBox
    //       ? item?.selectCategory?.price + COMMON_VALUE_FOR_VALUATION > 0
    //         ? 1
    //         : 0
        ...
        const allPriceValues = bulkFormsData?.map((item, index) => {
          ...
          if (
            ...
            if (item?.marketValuationCheckBox === true) {
              basePrice = COMMON_VALUE_FOR_VALUATION;
            } else {
              basePrice = 0;
            }
          } else if (
                    ...
                    {priceValues?.length > 0
                      ...
                      : (item?.marketValuationCheckBox
                          ? item?.selectCategory?.price +
                            COMMON_VALUE_FOR_VALUATION
                          : item?.selectCategory?.price) +
                        (item?.add_on_checkbox ? COMMON_VALUE_FOR_ADD_ON : 0)}
                  </Typography>
